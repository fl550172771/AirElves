...
+ (instancetype)alertControllerWithTitle:(NSString *)title message:(NSString *)message  AlertTag:(NSNumber*)tag preferredStyle:(MSAlertControllerStyle)preferredStyle {
    return [[[self class] alloc] initWithTitle:title message:message tag:tag preferredStyle:preferredStyle];
}

//Mod by ZhangPeihang 
- (id)initWithTitle:(NSString *)title message:(NSString *)message tag:(NSNumber*)tag preferredStyle:(MSAlertControllerStyle)preferredStyle {
    ...
    if (self) {
        ...
        self.alertTag = tag;//Add by ZhangPeihang
        if (preferredStyle == MSAlertControllerStyleAlert) {
            self.titleColor = [UIColor blackColor];
            self.titleFont = [UIFont fontWithName:@"HelveticaNeue-Bold" size:24.0f];
            self.messageColor = [UIColor blackColor];
            self.messageFont = [UIFont fontWithName:@"HelveticaNeue" size:1.0f];
        } else {
...
//        self.tableViewHeightConstraint.constant = tableViewHeight + headerHeight - 1.0f;
        self.tableViewHeightConstraint.constant = tableViewHeight - 1.0f;//pishum mod
    } else {
...
//        self.tableViewContainerHeightConstraint.constant = tableViewHeight + headerHeight + self.tableViewBottomSpaceConstraint.constant - 0.5f;
        self.tableViewContainerHeightConstraint.constant = tableViewHeight + self.tableViewBottomSpaceConstraint.constant - 0.5f;//pishum mod
